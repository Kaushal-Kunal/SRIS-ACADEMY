...
export class InsbatchComponent implements OnInit {
  displayedColumns: string[] = ['institute_id', 'institute_name', 'institute_owner', 'institute_whatsapp', 'institute_email', 'total_batch'];
  ...
  ngOnInit(): void {
    this.manageservice.batch_for_admin().subscribe(
      (instdata: any) => {
        console.log(instdata.data[0].total_batch)
  ...
  get_batch(row: any) {
    ...
    this.router.navigate(['/adminhome/insbatch/batch'], row.inst_id)
  }
